...
STRATEGY_DIR="/c/Users/admin/IWE/DS-strategy"

# Agent Workspace: если существует — отчёты идут туда
AGENT_WORKSPACE="/c/Users/admin/IWE/DS-agent-workspace"
...
"
        if echo "$warnings" | grep -q "push failed" 2>/dev/null; then
            report+="- **push failed:** Mac был оффлайн. Запусти \`cd /c/Users/admin/IWE/DS-strategy && git pull --rebase && git push\`
"
        fi
    else
        report+="
