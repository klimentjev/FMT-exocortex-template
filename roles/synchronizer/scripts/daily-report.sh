...
STRATEGY_DIR="/mnt/c/Users/admin/IWE/DS-strategy"

# Agent Workspace: если существует — отчёты идут туда
AGENT_WORKSPACE="/mnt/c/Users/admin/IWE/DS-agent-workspace"
...
STRATEGIST_LOG="/home/wklim/logs/strategist/$DATE.log"
...
"
        if echo "$warnings" | grep -q "push failed" 2>/dev/null; then
            report+="- **push failed:** Mac был оффлайн. Запусти \`cd /mnt/c/Users/admin/IWE/DS-strategy && git pull --rebase && git push\`
"
        fi
    else
        report+="
