...
Рабочая директория: `/c/Users/admin/IWE/` (не из sub-директорий). `/c/Users/admin/IWE/memory/` = симлинк на auto-memory.
...
> **3 слоя:** L1 (§1-§7) = платформа (обновление: `DS-exocortex/update.sh` при layout IWE+DS-exocortex). L2 (§8) = staging. L3 (§9) = авторское.
...
- `/c/Users/admin/IWE/` — рабочая директория
...
**Архитектурное обоснование:** платформенные файлы (L1) и пользовательские расширения (L3) -- разные слои. Смешение слоёв = хрупкость при обновлении. Разделение: платформенное → репозиторий шаблона на GitHub (`FMT-exocortex-template`) → применение обновлений локально через `DS-exocortex/update.sh` (или `update.sh` в корне экзокортекса, если каталог экзокортекса = корень репо). Пользовательское → `extensions/` + `params.yaml`.

**Для автора шаблона (`params.yaml → author_mode: true`):** прямое редактирование L1 файлов РАЗРЕШЕНО.
- **Flow:** авторский IWE (source-of-truth) → `template-sync.sh` → FMT (с плейсхолдерами) → GitHub → у пользователей `update.sh` / у автора в этом workspace — `DS-exocortex/update.sh`.
