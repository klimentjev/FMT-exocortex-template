...
> **Двухуровневый FAQ:** Доменный («что/зачем/как устроено») → `DP.IWE.002 §11` (Pack, source-of-truth для бота). Практический («как сделать/что делать если») → `docs/LEARNING-PATH.md §11`.
> При добавлении/изменении FAQ-вопроса:
> 1. Определи тип: доменный → `DP.IWE.002 §11` (полный ответ, в нужную категорию)
> 2. Или практический → `LEARNING-PATH.md §11` (строка в нужную категорию)
> 3. Если доменный → добавь краткую выжимку в `README.md §FAQ`
> 4. `selective-reindex exocortex-template-docs` (бот найдёт через knowledge-mcp)
>
> **Категории Pack FAQ:** Пререквизиты и стоимость · Платформа и совместимость · Компоненты и архитектура · Кастомизация · Установка и устранение проблем.
> **Категории LP Quick Ref:** Протоколы и рабочий процесс · Репозитории и структура · Знания и Pack · Навигация и инструменты.
