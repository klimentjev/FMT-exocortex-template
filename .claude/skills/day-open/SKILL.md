...
Коммиты за вчера по всем `/c/Users/admin/IWE/*/` репо. Сопоставить с DayPlan.
...
**Проверка обновлений:** `cd /c/Users/admin/IWE/FMT-exocortex-template && bash update.sh --check 2>&1`. Если доступно обновление → добавить в «Требует внимания»: «Доступно обновление IWE → `/iwe-update`».

**Проверка Base-репо (FPF, SPF, ZP):**
```bash
for repo in FPF SPF ZP; do
  dir="/c/Users/admin/IWE/$repo"
...
```
Если есть новые коммиты → добавить в «Требует внимания»: «[repo] обновлён upstream → `cd /c/Users/admin/IWE/[repo] && git pull --rebase`». После pull FPF/SPF → reindex: `bash /c/Users/admin/IWE/DS-MCP/knowledge-mcp/scripts/selective-reindex.sh FPF` (или SPF).
